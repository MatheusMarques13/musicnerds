...
export function AlbumDetailView({ albumId }: { albumId: string }) {
  ...
  useEffect(() => {
    ...
    supabase.from('ratings').select('rating').eq('user_id', user.id).eq('spotify_id', albumId).single()
  ...
  async function handleRate(stars: number) {
    ...
    await supabase.from('ratings').upsert({
      user_id: user.id, spotify_id: albumId,
      album_title: album?.title, artist: album?.artist, cover_url: album?.cover_url,
      rating: stars, updated_at: new Date().toISOString(),
  ...
  async function handleAddToCollection() {
    ...
    await supabase.from('collection').upsert({
      user_id: user.id, spotify_id: albumId,
      album_title: album?.title, artist: album?.artist, cover_url: album?.cover_url, status: 'listened',
    }, { onConflict: 'user_id,spotify_id' })
    setSaved(true)
    setTimeout(() => setSaved(false), 2000)
  }
  ...
  if (!album) return (
    <div className="flex flex-col items-center justify-center h-screen gap-4">
      ...
      <button onClick={() => router.back()} className="text-sm" style={{ color: 'var(--accent)' }}>Go back</button>
    </div>
  )
    ...
    <div className="max-w-4xl mx-auto">
      ...
      >
        <div className="flex flex-col sm:flex-row gap-8">
          {/* Cover */}
          <div className="relative w-44 h-44 shrink-0 rounded-2xl overflow-hidden mx-auto sm:mx-0" style={{ boxShadow: 'var(--shadow-hover)' }}>
            {album.cover_url ? (
              <Image src={album.cover_url} alt={album.title} fill className="object-cover" sizes="176px" />
            ) : (
              <div className="w-full h-full flex items-center justify-center font-serif text-white text-4xl" style={{ backgroundColor: 'var(--accent)' }}>
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-2" style={{ color: 'var(--accent)' }}>Album</p>
              <h1 className="font-serif text-4xl font-bold leading-tight mb-1" style={{ color: 'var(--text)' }}>{album.title}</h1>
              ...
              <div className="flex flex-wrap gap-3 text-xs mb-4" style={{ color: 'var(--muted)' }}>
                ...
                {album.total_tracks > 0 && <span><Music2 size={12} className="inline mr-1" />{album.total_tracks} tracks</span>}
                {lastfm?.listeners && <span>👥 {Number(lastfm.listeners).toLocaleString()} listeners</span>}
                {lastfm?.playcount && <span>▶️ {Number(lastfm.playcount).toLocaleString()} plays</span>}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div>
                ...
                <div className="flex gap-1.5 items-center">
                  ...
                  {saved && <span className="text-xs ml-1" style={{ color: 'var(--accent)' }}>Saved ✓</span>}
                </div>
              </div>
        ...
        >
          ...
          <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
            {tracks.map((track: any) => (
              ...
              >
                <span className="text-xs w-5 text-right shrink-0" style={{ color: 'var(--muted)' }}>{track.number}</span>
                <span className="flex-1 text-sm font-medium truncate" style={{ color: 'var(--text)' }}>{track.title}</span>
